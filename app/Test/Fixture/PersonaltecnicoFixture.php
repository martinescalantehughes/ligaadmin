<?php
/**
 * PersonaltecnicoFixture
 *
 */
class PersonaltecnicoFixture extends CakeTestFixture {

/**
 * Fields
 *
 * @var array
 */
	public $fields = array(
		'id' => array('type' => 'integer', 'null' => false, 'default' => null, 'unsigned' => false, 'key' => 'primary'),
		'cargo' => array('type' => 'string', 'null' => true, 'default' => null, 'length' => 250, 'collate' => 'utf8_general_ci', 'charset' => 'utf8'),
		'funcion' => array('type' => 'string', 'null' => true, 'default' => null, 'length' => 250, 'collate' => 'utf8_general_ci', 'charset' => 'utf8'),
		'fechaafiliacion' => array('type' => 'date', 'null' => true, 'default' => null),
		'fecharenovacion' => array('type' => 'date', 'null' => true, 'default' => null),
		'persona_id' => array('type' => 'integer', 'null' => false, 'default' => null, 'unsigned' => false, 'key' => 'index'),
		'ligachaquenia_id' => array('type' => 'integer', 'null' => false, 'default' => null, 'unsigned' => false, 'key' => 'index'),
		'indexes' => array(
			'PRIMARY' => array('column' => 'id', 'unique' => 1),
			'fk_Personal_Tecnico_Persona1_idx' => array('column' => 'persona_id', 'unique' => 0),
			'fk_Personal_Tecnico_LigaChaqueña1_idx' => array('column' => 'ligachaquenia_id', 'unique' => 0)
		),
		'tableParameters' => array('charset' => 'utf8', 'collate' => 'utf8_general_ci', 'engine' => 'InnoDB')
	);

/**
 * Records
 *
 * @var array
 */
	public $records = array(
		array(
			'id' => 1,
			'cargo' => 'Lorem ipsum dolor sit amet',
			'funcion' => 'Lorem ipsum dolor sit amet',
			'fechaafiliacion' => '2016-03-26',
			'fecharenovacion' => '2016-03-26',
			'persona_id' => 1,
			'ligachaquenia_id' => 1
		),
	);

}
